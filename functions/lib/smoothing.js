...
exports.smoothreadings = exports.kalmanFilter = void 0;
 ...
 */
function kalmanFilter(z, x_est_prev, P_prev, Q = 0.01, R = 0.1) {
    // Prediction Step
 ...
 */
exports.smoothreadings = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const { data: tank, error: tankError } = await supabase_1.supabase
            .from('tanks')
            .select('last_smoothed_level, filter_covariance, outlier_count')
            .eq('id', tank_id)
            .single();
        if (tankError || !tank)
            throw new Error(`Tank ${tank_id} not found`);
        ...
        if (tank.last_smoothed_level !== undefined && deviation > outlierThreshold && deviation > minDeviationFloor) {
            ...
            await supabase_1.supabase.from('tanks').update(updatePayload).eq('id', tank_id);
            res.status(200).send(newOutlierCount >= 5 ? 'Filter reset after multiple outliers' : 'Outlier detected and logged');
            return;
        }
        // Apply Kalman Filter
        const { x_est: newSmoothedLevel, P: newCovariance } = kalmanFilter(ambient_volume, prevEstimate, prevCovariance, 0.01, // Process noise
