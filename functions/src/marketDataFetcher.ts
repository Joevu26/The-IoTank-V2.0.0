 ...
 */
export const fetchMarketData = functions.pubsub
 ...
 */
async function fetchEIAData(EIA_API_KEY: string) {
    try {
        ...
        const response = await axios.get(brentUrl);
        ...
        if (data) {
            const { error } = await supabase.from('market_signals').insert({
                ...
                confidence: 1.0, // Official Govt Source
                validated: true,
                data_points: {
 ...
 */
async function fetchAlphaVantageData(ALPHA_VANTAGE_KEY: string) {
    try {
        ...
        const wtiResponse = await axios.get(wtiUrl);

        if (wtiResponse.data?.data?.[0]) {
 ...
 */
async function fetchExchangeRateData(apiKey: string) {
    try {
        ...
        if (response.data && response.data.result === 'success') {
            ...
            const { error } = await supabase.from('market_signals').insert({
                ...
                data_points: {
                    ...
                    unit: 'KES/USD', // Using KES per USD as standard
                    base: 'USD',
                },
                source_type: 'API',
