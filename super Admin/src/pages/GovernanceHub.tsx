...
const GovernanceHub: React.FC = () => {
    ...
    const sections = [
        { id: 'audit', label: 'Compliance Audit', icon: <FiShield /> },
        { id: 'logs', label: 'Admin Activity', icon: <FiFileText /> }
    ];
        ...
        <Layout>
            <div className="fleet-hub-container">
                ...
                <div className="hub-content-surface animate-fade-in">
                    {activeSection === 'audit' && <AuditCompliance isHubView={true} />}
                    {activeSection === 'logs' && <AdminLogs isHubView={true} />}
                </div>
            </div>
        </Layout>
    );
